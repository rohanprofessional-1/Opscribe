...
export default function SettingsModal({ isOpen, onClose, initialTab }: SettingsModalProps) {
    ...
    const fetchRepos = async () => {
        if (!clientId) {
            console.log("fetchRepos() aborted: Client ID not found.");
            return;
        }
        setLoadingRepos(true);
        try {
            console.log(`fetchRepos(): Hitting GET /github/repos?client_id=${clientId}`);
            const res = await fetch(`${API_BASE}/github/repos?client_id=${clientId}`);
            if (res.ok) {
                const data = await res.json();
                console.log("fetchRepos() SUCCESS Data:", data);
                setRepos(data);
            } else {
                const text = await res.text();
                console.error("fetchRepos() FAILED response:", res.status, text);
            }
        } catch (error) {
            console.error("fetchRepos() EXCEPTION:", error);
        } finally {
            console.log("fetchRepos() finished, loading set to false.");
    ...
    const handleSaveGitHub = async () => {
        ...
        try {
            ...
            // Re-fetch the repositories immediately so the UI updates if auto-discovery linked the installation
            await fetchRepos();

            // Refresh config to get the install URL
            fetch(`${API_BASE}/github/config?client_id=${clientId}`)
                .then(r => r.json())
                .then(d => setGithubInstallUrl(d.app_install_url));

            setGhPrivateKey(""); // clear sensitive fields from UI
            setGhWebhookSecret("");
        } catch (e: any) {
        ...
        try {
            await fetch(`${API_BASE}/github/connect`, {
                ...
                body: JSON.stringify({
                    ...
                    target_repo_id: repo.id.toString(),
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 w-full min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400 mb-4 leading-relaxed">
                                            Opscribe requires a dedicated GitHub App installed on your Organization. To create a new app or configure an existing one, go to <strong>Settings {">"} Developer Settings {">"} GitHub Apps</strong> and ensure the following values are set:
                                        </p>
                                        <div className="grid grid-cols-1 gap-2 text-xs mb-4">
                                            <div className="bg-gray-950 p-2.5 rounded border border-gray-800 flex justify-between items-center group w-full">
                                                ...
                                                <div className="flex items-center gap-2 overflow-hidden flex-1 justify-end">
                                                    ...
                                                    <button onClick={() => handleCopy(window.location.origin, 'home')} className="text-gray-500 hover:text-white shrink-0 p-1">
                                                        {copiedStates['home'] ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
                                                    </button>
                                                </div>
                                            </div>
                                            <div className="bg-gray-950 p-2.5 rounded border border-gray-800 flex justify-between items-center group w-full">
                                                <span className="text-gray-500 w-32 shrink-0">Setup URL (Post-Install)</span>
                                                <div className="flex items-center gap-2 overflow-hidden flex-1 justify-end">
                                                    ...
                                                    <button onClick={() => handleCopy(`${window.location.origin}/api/github/app/callback?client_id=${clientId}`, 'callback')} className="text-gray-500 hover:text-white shrink-0 p-1">
                                                        {copiedStates['callback'] ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
                                                    </button>
                                                </div>
                                            </div>
                                            <div className="bg-gray-950 p-2.5 rounded border border-gray-800 flex justify-between items-center group w-full">
                                                ...
                                                <div className="flex items-center gap-2 overflow-hidden flex-1 justify-end">
                                                    ...
                                                    <button onClick={() => handleCopy(`${window.location.origin}/api/github/webhook?client_id=${clientId}`, 'webhook')} className="text-gray-500 hover:text-white shrink-0 p-1">
                                                        {copiedStates['webhook'] ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="mb-6 bg-gray-800/50 border border-gray-700 rounded-xl p-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            <div>
                                                <label className="block text-xs text-gray-500 mb-1">App Name (Slug) *</label>
                                                <input type="text" value={ghAppSlug} onChange={e => setGhAppSlug(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2.5 text-sm text-white focus:border-blue-500 outline-none" placeholder="my-company-opscribe" />
                                            </div>
                                            <div>
                                                <label className="block text-xs text-gray-500 mb-1">App ID *</label>
                                                <input type="text" value={ghAppId} onChange={e => setGhAppId(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2.5 text-sm text-white focus:border-blue-500 outline-none" placeholder="123456" />
                                            </div>
                                        </div>
                                        <div className="mb-4">
                                            <label className="block text-xs text-gray-500 mb-1">Webhook Secret (Optional)</label>
                                            <input type="password" value={ghWebhookSecret} onChange={e => setGhWebhookSecret(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2.5 text-sm text-white focus:border-blue-500 outline-none" placeholder="Enter the secret you created..." />
                                        </div>
                                        <div className="mb-4">
                                            <label className="block text-xs text-gray-500 mb-1">Private Key (PEM) *</label>
                                            <textarea value={ghPrivateKey} onChange={e => setGhPrivateKey(e.target.value)} rows={4} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2.5 text-xs font-mono text-gray-300 focus:border-blue-500 outline-none" placeholder="-----BEGIN RSA PRIVATE KEY-----&#10;...&#10;-----END RSA PRIVATE KEY-----" />
                                        </div>
                                        ...
                                        <div className="flex justify-end">
                                            <button onClick={handleSaveGitHub} disabled={isSavingGh} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50">
                                                {isSavingGh ? "Saving..." : "Save Credentials"}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                                    ...
                                                    <div key={repo.id} className="bg-gray-800 rounded-lg border border-gray-700 p-4 flex items-center justify-between">
                                                        <div>
                                                            <div className="flex items-center gap-2 mb-1">
                                                                ...
                                                                <a href={`https://github.com/${repo.name}`} target="_blank" rel="noreferrer" className="text-sm font-medium text-gray-200 hover:text-white hover:underline truncate max-w-[250px]">
                                                                    {repo.name}
                                                                </a>
                                                            </div>
                                                            <div className="text-xs text-green-400 flex items-center gap-1.5 mt-2 font-medium">
                                                                ✓ Connected
                                                            </div>
                                                        </div>
